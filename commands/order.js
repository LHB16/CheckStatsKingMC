    ...
    try {
      ...
      if (orders.length === 0) {
        ...
        const emptyEmbed = new EmbedBuilder()
          .setTitle(`${chestEmoji} Đơn hàng: **${itemQuery}**`)
          .setDescription(`${barrierEmoji} Không tìm thấy Order cho món đồ **${itemQuery}**.`)
          .setColor('#ef4444')
          .setTimestamp()
      ...
      if (displayMode === 'image') {
        ...
        for (let attempt = 1; attempt <= 2; attempt++) {
          try {
            ...
            lastError = renderErr;
            console.error(`[Discord-Bot] Lần thử ${attempt} batch render ảnh Order lỗi:`, renderErr.message);
          }
        }

        if (imageBuffers && imageBuffers.length > 0) {
          const page1Buffer = imageBuffers[0];
          const attachment = new AttachmentBuilder(page1Buffer, { name: 'order_table_p1.png' });
      ...
      const textTitle = totalPages > 1
        ? `${chestEmoji} Danh sách Order: **${itemQuery}** (Trang 1/${totalPages})`
        : `${chestEmoji} Danh sách Order: **${itemQuery}**`;
      ...
      const errorEmbed = new EmbedBuilder()
        .setTitle(`${barrierEmoji} Lỗi kiểm tra đơn hàng`)
